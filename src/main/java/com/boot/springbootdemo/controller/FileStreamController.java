...
public class FileStreamController {
    ...
    public ResultVO testMybatisPlus(){
        List<GoodsInfoBean> goodsInfoBeans = goodsInfoMapper.selectList(Wrappers.<GoodsInfoBean>lambdaQuery()
                .select(GoodsInfoBean::getName,GoodsInfoBean::getPrice,GoodsInfoBean::getDescription)
                .eq(GoodsInfoBean::getId,"1003"));

        ResultVO resultVO = new ResultVO();
        resultVO.setCode(200+"");
        resultVO.setMessage(goodsInfoBeans);

        goodsInfoBeans.forEach(e ->{
            System.out.println(e.getDescription());
        });
